...
def prob6(list=[2,8,7,2,2,5,2,3,1,2,2]):
    ...
    for number in list:
        if count == 0:
            candidat = number
        count += (1 if number == candidat else -1)
    if list.count(candidat) > len(list) // 2:
        return candidat
    return None
...
def prob6_with_gpt(lista=[2,8,7,2,2,5,2,3,1,2,2]):
    ...
    return max(counts, key=counts.get) if counts[max(counts, key=counts.get)] > len(lista) // 2 else None
