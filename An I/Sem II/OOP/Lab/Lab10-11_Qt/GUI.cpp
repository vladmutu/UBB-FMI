...
void GUI::reloadList(QListWidget* listaMasini, const vector<Masina>& masini)
{
	listaMasini->clear();
	for (const auto& masina : masini)
	{
		QListWidgetItem* item = new QListWidgetItem(QString::fromStdString(masina.get_nr_inmatriculare() + ", " + masina.get_producator() + ", " + masina.get_model() + ", " + masina.get_tip()));
		listaMasini->addItem(item);
	}
	initButtons();
}
