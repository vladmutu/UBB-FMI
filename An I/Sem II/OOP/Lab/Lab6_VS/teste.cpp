...
void testAddRepo() {
    ...
    assert(repo.getAllMasini().get(0).get_nr_inmatriculare() == "BH-04-VLD");
    assert(repo.getAllMasini().get(1).get_producator() == "Skoda");
    assert(repo.getAllMasini().get(2).get_model() == "Octavia");
    assert(repo.getAllMasini().get(3).get_tip() == "Sedan");
    try {
        repo.adaugaMasina(m1);
        assert(false);
    }
...
void testStergeMasina()
{
    ...
    assert(repo.getAllMasini().get(0).get_nr_inmatriculare() == "BH-04-VLD");
    assert(repo.getAllMasini().get(1).get_nr_inmatriculare() == "BH-06-VLD");
    try {
...
void testModificaMasina()
{
    ...
    assert(repo.getAllMasini().get(1).get_tip() == "Combi");
    try {
...
void testModificaMasinaService()
{
    ...
    assert(service.getAllMasiniService().get(0).get_model() == "Fabia");
    model = "Octavia";
    service.modificaMasinaService(nr_inmatriculare, producator, model, tip);
    nr_inmatriculare = "BH-05-VLD";
    try {
...
void testFilterProducator()
{
    ...
    assert(masini.get(0).get_producator() == "Skoda");
    assert(masini.get(1).get_producator() == "Skoda");
    masini = service.filterMasiniProducer(producator2);
    assert(masini.size() == 1);
    assert(masini.get(0).get_producator() == "Dacia");
}
...
void testFilterTip()
{
    ...
    assert(masini.size() == 2);
    assert(masini.get(0).get_tip() == "Sedan");
    assert(masini.get(1).get_tip() == "Sedan");
    masini = service.filterMasiniType(tip2);
    assert(masini.size() == 1);
    assert(masini.get(0).get_tip() == "Combi");
}
...
void testSortare()
{
    ...
    assert(lista_sortata.get(0).get_nr_inmatriculare() == "BH-04-VLD");
    assert(lista_sortata.get(1).get_nr_inmatriculare() == "BH-05-VLD");
    lista_sortata = service.sortMasini(sortTip);
    assert(lista_sortata.get(0).get_tip() == "Break");
    assert(lista_sortata.get(1).get_tip() == "Sedan");
    lista_sortata = service.sortMasini(sortProducatorModel);
    assert(lista_sortata.get(0).get_producator() == "Dacia");
    assert(lista_sortata.get(1).get_producator() == "Skoda");
    nr_inmatriculare = "BH-06-VLD";
    producator = "Dacia";
    model = "Logam";
    tip = "Break";
    ...
    lista_sortata = service.sortMasini(sortProducatorModel);
    assert(lista_sortata.get(0).get_producator() == "Dacia");
    assert(lista_sortata.get(1).get_producator() == "Dacia");
    assert(lista_sortata.get(2).get_producator() == "Skoda");
    assert(lista_sortata.get(0).get_model() == "Logam");
    assert(lista_sortata.get(1).get_model() == "Logan");
}
...
void testAssignmentOperator() {
    ...
    // Test deep copy
    VectorDinamic<int> v2 = v1;
    assert(v2.size() == v1.size());
    for (int i = 0; i < v1.size(); i++) {
        assert(v2.get(i) == v1.get(i));
    }
...
void testSortProducator()
{
    ...
    tip = "Break";
    service.adaugaMasinaService(nr_inmatriculare, producator, model, tip);
    VectorDinamic<Masina> lista_sortata = service.sortMasini(sortProducator);
    assert(lista_sortata.get(0).get_producator() == "Dacia");
    assert(lista_sortata.get(1).get_producator() == "Skoda");
}
...
void testSortModel()
{
    ...
    tip = "Break";
    service.adaugaMasinaService(nr_inmatriculare, producator, model, tip);
    VectorDinamic<Masina> lista_sortata = service.sortMasini(sortModel);
    assert(lista_sortata.get(0).get_model() == "Logan");
    assert(lista_sortata.get(1).get_model() == "Octavia");
}
