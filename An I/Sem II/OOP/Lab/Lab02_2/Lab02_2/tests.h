//
// Created by vladm on 3/13/2024.
//header file pentru functiile de test
#pragma once

/*
 * functie de test pentru crearea unui electrocasnic
 */
void testCreateElectronic();

/*
 * functie de test pentru validator electrocasnic
 */
void testValidateElectronic();

/*
 * functie de test pentru adaugarea in repo a unui electrocasnic
 */
void testAdd();

/*
 * functie de test pentru marirea dimeansiunii vectorului dinamic
 */
void testResize();

/*
 *functie de test pentru crearea unei liste
 * care sa fie goala
 */
void testCreateEmpty();

/*
 * functie de test pentru stergerea unui element din lista de electrocasnice
 */
void testDelete();

/*
 * functie de test pentru service adaugare electrocasnic
 */
void testAddElectronicService();

/*
 * functie de test service pentru actualizarea unui element din lista
 */
void testUpdateElectronicService();

/*
 * functie de test service pentru stergerea unui electrocasnic din lista
 */
void testDeleteElectronicService();

/*
 * functie de test pentru actualizarea cantitatii unui electrocasnic
 */
void testUpdateQuantity();

/*
 * functie de test pentru destroy list
 */
void testDestroy();

/*
 * functie de test pentru destroy electronic
 */
void testDestroyElectronic();

/*
 * functie de test pentru sortarea crescatoare a electrocasnicelor dupa pret
*/
void testSortElectronicsPriceAscending();

/*
 * functie de test pentru sortarea descrescatoare a electrocasnicelor dupa pret
*/
void testSortElectronicsPriceDescending();

/*
* functie de test pentru sortarea crescatoare a electrocasnicelor dupa cantitate
*/
void testSortElectronicsQuantityAscending();

/*
* functie de test pentru sortarea descrescatoare a electrocasnicelor dupa cantitate

*/
void testSortElectronicsQuantityDescending();

/*
 * functie de test pentru copierea unui electronic
 */
void testCopyElectronic();

/*
 *functie de test pentru size
 */
void testSize();

/*
 * functie de test pentru set
 */
void testSet();

/*
 * functie de test pentru sort
 */
void testSort();

/*
 * functie de test pentru crearea unui list manager
 */
void testCreateListManager();

/*
 * functie de test pentru filtrarea electrocasnicelor dupa producator
 */
void testFilterByProducer();

/*
 * functie de test pentru filtrarea electrocasnicelor dupa tip
 */
void testFilterByPrice();

/*
 * functie de test pentru filtrarea electrocasnicelor dupa cantitate
 */
void testFilterByQuantity();

/*
 * functie de test pentru undo
 */
void testUndoService();

/*
 * functie de rularea a tuturor testelor
 */
void runAllTests();